...
class LogFermiSphericalWallPotential(BiasPotential):
    ...
    def _get_bias_energy_and_force(self, atoms):
        E, E_grad = _ext.log_fermi_spherical_potential(
            atoms.positions, self.radius, self.temperature, self.beta
        )
        return E, -E_grad
    ...
    def todict(self):
        ...
        return {
            ...
            "kwargs": {
                ...
                "bondlengths": self.bondlengths.tolist()
                if self.bondlengths is not None
                else None,
            },
        }
...
def _adjust_positions(
    pairs, bondlengths, masses, old, new, maxiter, tolerance, cell, pbc
):
...
@njit(fastmath=True)
def _adjust_momenta(
    pairs, bondlengths, masses, old, momenta, maxiter, tolerance, cell, pbc
):
    for _ in range(maxiter):
        converged = True
        for i, (a, b) in enumerate(pairs):
